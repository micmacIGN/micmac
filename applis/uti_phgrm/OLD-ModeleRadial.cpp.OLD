...
int main(int argc,char ** argv)
{
	...
	LArgMain()  << EAM(aDir)
	            ...
	            << EAM(aName2) 
	            << EAM(aRes) ,
	LArgMain()  << EAM(aKey,"Key",true)
                    << EAM(aKeyStd,"KeyStd",true)
    ...
    if (aKeyStd!="")
    {
       aKey =  + "KeyStd-Assoc-Im2Orient@" + aKeyStd;
    }
    else 
    {
        if (aKey=="")
        {
           aKey = "Key-Assoc-Im2OrInit";
        }
    }
    ...
    cInterfChantierNameManipulateur * anICNM = cInterfChantierNameManipulateur::StdAlloc
     ...
     aCplE.ImEpip(Tiff_Im::StdConv(aDir+aName1),true,aDir+"EpiGauche_"+aRes);
     aCplE.ImEpip(Tiff_Im::StdConv(aDir+aName2),false,aDir+"EpiDroite_"+aRes);
