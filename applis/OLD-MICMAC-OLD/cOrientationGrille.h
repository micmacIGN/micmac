...
#define nomGrille std::string("Grille")


class OrientationGrille: public ModuleOrientation
{
public:
  OrientationGrille(std::string const &nom);
  virtual ~OrientationGrille(){std::cout << "Destructeur de OrientationGrille" << std::endl;}
  virtual void ImageAndPx2Obj(double c, double l, const double *aPx,
			      double &x, double &y)const;
  virtual void Objet2ImageInit(double x, double y, const double *aPx,
...
private:
  ...
  std::complex<double> interpolation(std::complex<double> position_grille,
		      std::vector<double> const &grille,std::complex<int> Taille, int TailleZ) const;
