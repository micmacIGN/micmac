    ...
    mMask           ("")
{
    ...
    ElInitArgMain
        (
            ...
            LArgMain() << EAM(mTargetRate,"Rate",true,"Rate final Def=4")
                       ...
                       << EAM(mMask,"Mask",true,"Global mask to filter tie points")
...

    {
        ...
        if (!mMask.empty())
        {
            std::string aMasking = MM3dBinFile("HomolFilterMasq ")
                                   + QUOTE(aMMPatImOk) + " "
                                   + std::string("GlobalMasq=") + QUOTE(mMask);
            System(aMasking);

            std::string aRmHomol, aMvHomolMasqFiltered;

            if (isWindows){
                aRmHomol = "rmdir /s /q Homol";
                aMvHomolMasqFiltered = "move HomolMasqFiltered Homol";
            } else {
                aRmHomol = "rm -rf Homol";
                aMvHomolMasqFiltered = "mv HomolMasqFiltered Homol";
            }
            System(aRmHomol);
            System(aMvHomolMasqFiltered);
        }
    }
