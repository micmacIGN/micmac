...
class cAppli_CalculCalibRadiom
{
    public :
        cAppli_CalculCalibRadiom(int,char **);

        void Moy(Im2DGen *);

        std::string mNameToCal;
        std::string mNameCal;
        int         mDeg;
        int         mSzW;
        Pt2di       mDecal;
        Tiff_Im     *mTif2Cal;
        Tiff_Im     *mTifCal;
        std::vector<Im2DGen *>   mVecIm2Cal;
        std::vector<Im2DGen *>   mVecImCal;
          
};

void cAppli_CalculCalibRadiom::Moy(Im2DGen * anIm)
{
    Fonc_Num aF = Rconv(anIm->in(0));
    int aSzW2 = mSzW/2;
    for (int aK=0 ; aK< 4 ; aK++)
        aF  = rect_som(aF * anIm->inside(),aSzW2) / Max(1.0,rect_som(anIm->inside(),aSzW2));
...
cAppli_CalculCalibRadiom::cAppli_CalculCalibRadiom(int argc,char ** argv) :
   mDeg (1),
   mSzW (20)
{
    ElInitArgMain
    (
         argc,argv,
         LArgMain()  << EAMC(mNameToCal,"Prototype file to calibrate")
                     << EAMC(mNameCal,"Homologous calibrated image")
                     << EAMC(mDecal,"Decal"),
         LArgMain()  << EAM(mDeg,"Deg",true,"Degree of Polynome")
    );

     mTif2Cal = new Tiff_Im(Tiff_Im::StdConvGen(mNameToCal,-1,true));
     mTifCal = new Tiff_Im(Tiff_Im::StdConvGen(mNameCal,-1,true));


     mVecIm2Cal = mTif2Cal->ReadVecOfIm();
     mVecImCal = mTifCal->ReadVecOfIm();

     int aNbC = mVecIm2Cal.size();
     ELISE_ASSERT(aNbC==int(mVecImCal.size()),"Diff nb chan in images");
     ...
     {
         Im2DGen * aIm2Cal = mVecIm2Cal[aKC];
         Im2DGen * aImCal = mVecImCal[aKC];
         Moy(aIm2Cal);
         Moy(aImCal);
         Pt2di aSz2Cal = aIm2Cal->sz();
         Pt2di aSzCal = aImCal->sz();
         Box2di aBoxCal(Pt2di(0,0),aSzCal);


         Pt2di aP2Cal;
         L2SysSurResol aSys(mDeg+1);
         ElPolynome<double> aPol((char*)0,mDeg);
         for (int anIter=0; anIter<2 ; anIter++)
         {
             bool IterCalc = (anIter==0);
             double aMoyRes= 0;
             std::vector<double> aVRes;
             for (aP2Cal.x=0 ; aP2Cal.x<aSz2Cal.x ; aP2Cal.x+=mSzW)
             {
                 for (aP2Cal.y=0 ; aP2Cal.y<aSz2Cal.y ; aP2Cal.y+=mSzW)
                 {
                     Pt2di aPCal = aP2Cal - mDecal;
                     if (aBoxCal.inside(aPCal))
                     {
                         double aV2Cal = aIm2Cal->GetR(aP2Cal);
                         double aVCal = aImCal->GetR(aPCal);
                         if (IterCalc)
                         {
                            double aPVC = 1;
                            std::vector<double> aVP;
                            for (int aD=0 ; aD<= mDeg ; aD++)
                            {
                                aVP.push_back(aPVC);
                                aPVC *= aV2Cal;
                                aSys.AddEquation(1.0,VData(aVP),aVCal);
                            }
                         }
                         else
                         {
                            double aRes = ElAbs(aVCal - aPol(aV2Cal));
             ...
             if (IterCalc)
             {
                ...
                aPol = ElPolynome<double>(aSol.data(),mDeg);
                if (0)
                {
                   for (int aK=0 ; aK<=mDeg; aK++)
                       std::cout << "SOOlll = " << aSol.data()[aK] << "\n";
                }
             }
             else
             {
                std::cout << " =========== For chanel " << aKC << " ============== \n";
                std::cout << "   Residu  :: moy  = " << aMoyRes / aVRes.size() << "\n";
             }
         }

         Fonc_Num   aFTest1C = 0.0;
         Symb_FNum  aF2C(mTif2Cal->in().kth_proj(aKC));
         Fonc_Num   aPF2C = 1.0;
         for (int aD=0 ; aD<=mDeg ; aD++)
         {
             aFTest1C = aFTest1C + aPF2C * aPol[aD];
             aPF2C = aPF2C * aF2C;
         }
         aFTest = (aKC==0) ? aFTest1C : Virgule(aFTest,aFTest1C);

     }
     Tiff_Im::CreateFromFonc("TestCalibRadiom.tif",mVecIm2Cal[0]->sz(),aFTest,GenIm::real4);
}


int CPP_CalculCalibRadiom(int argc,char ** argv) 
{
    cAppli_CalculCalibRadiom anAppli(argc,argv);
