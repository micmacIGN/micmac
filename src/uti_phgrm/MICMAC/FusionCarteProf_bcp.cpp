...
template <class Type> double  cLoadedCP<Type>::PdsLinear(const Pt2dr & aPTer) const
{
   ...
   if (aPds > 0)
   {
       if (mHasCorrel)
       {
           aPds = mTImCorrel.getprojR(aPIm);//gets the interpolated value of the corrlation
           aPds = aPds / 256.0;
/*
...
*/
   ...
   for (aQ0.y = 0 ; aQ0.y < mSzCur.y; aQ0.y++)
   {
        ...
        for (aQ0.x = 0 ; aQ0.x < mSzCur.x; aQ0.x++)
        {
            ...
            aTIm0.oset(aQ0,-1);//set the value of the cube aTIm0 at aQ0 to -1
            aTImNb.oset(aQ0,0);//set the value of the cube aTImNb at aQ0 to -0
            bool isDebug = DebugActif && IsPBug(aQ0);
            if (aSomP>0)
            {
                for (int aKP=0 ; aKP < int(aPCel.size()) ; aKP++)//iterate over all heaps (depth maps)
                {
                    aPCel[aKP].SetPdsPile(aPCel[aKP].P() * (aPCel.size()/aSomP));//update the weight
                }
