...
int Bascule_main(int argc,char ** argv)
{
    ...
    LArgMain()
                    ...
                    << EAM(aP1Rep,"P1Rep",false,"P1Rep")
                    << EAM(aP2Rep,"P2Rep",false,"P2Rep")
                    ...
                    << EAM(DistFE,"DistFS",false,"Distance between to fix scale, if not given no scaling")
                    << EAM(Normal,"Norm",false,"Target normal for the plane")
                    << EAM(SNormal,"SNorm",false,"\"Symbolic Normal\" (must be X, Y or Z)")
                    << EAM(aLimBsH,"LimBsH",false,"Limit ratio base to high (Def=1e-2)")
