...
ElRotation3D TestOriConvention(std::string& aNameOri,
                               const std::string& mNameIm1,
                               const std::string& mNameIm2);

/* contains both triplets and couples;
   if couples then aC3=0 */
struct TripleStr {
   public:
    TripleStr(const CamStenope* aC1, const int aId1, const CamStenope* aC2,
              const int aId2, const CamStenope* aC3 = 0, const int aId3 = 0)
        : mC1(aC1), mC2(aC2), mC3(aC3), mId1(aId1), mId2(aId2), mId3(aId3) {}
...
class cAppliFictObs : public cCommonMartiniAppli {
   ...
   private:
    ...
    bool CalculateEllipseParam3(cXml_Elips3D& anEl,
                                std::vector<const CamStenope*>& aVC,
                                const std::string& aName1,
                                const std::string& aName2,
                                const std::string& aName3, int& NbPts);
    bool CalculateEllipseParam2(cXml_Elips3D& anEl,
                                std::vector<const CamStenope*>& aVC,
                                const std::string& aName1,
                                const std::string& aName2, int& NbPts);
    ...
    void FromHomolToMap(std::vector<const CamStenope*>& aVC,
                        const std::string& aName1, const std::string& aName2,
                        const std::string& aName3, tMapM& aMap);
    void AddVPts2Map(tMapM& aMap, ElPackHomologue& aPack, int anInd1,
                     int anInd2);
    ...
    void GenerateFicticiousObsInEl(cGenGaus3D& aGG1, std::vector<Pt3dr>& aVP,
                                   double aRedFac = 1);

    bool IsInSz(Pt2dr&) const;

    void UpdateAR(const ElPackHomologue*, const ElPackHomologue*,
                  const ElPackHomologue*, const int);
    void UpdateAROne(const ElPackHomologue*, const CamStenope*,
                     const CamStenope*, const int, const int);
    Pt2di ApplyRedFac(Pt2dr&);

    void SaveHomolOne(std::vector<int>& aId, std::vector<Pt2dr>& aPImV,
                      std::vector<float>& aAttr);
    void SaveHomol(std::string&);
    double CalcPoids(double aPds);

    bool CalcRedFac(const std::vector<const CamStenope*>& aVC,
                    const std::vector<Pt3dr>& aVP, const Pt3dr& aCDG,
                    double& aRedOut);
    void FilterPtOutOfImg(const std::vector<const CamStenope*>&, const Pt3dr&,
                          const std::pair<int, TripleStr*>&, std::vector<int>&,
                          std::vector<int>&, std::vector<Pt2dr>&);
    ...
    std::map<std::string, std::map<std::string, std::string>>
        mHomMap;  // cam name, cam name, hom name
    ...
    const std::vector<std::string>*
        mSetName;  // redundant with mNameMap; best to remove and be coherent
    std::string mDir;
    std::string mPattern;
    std::string mPrefHom;
    std::string mOut;
...
cAppliFictObs::cAppliFictObs(int argc, char** argv)
    ...
    ElInitArgMain(
        argc, argv, LArgMain() << EAMC(mPattern, "Pattern of images"),
        LArgMain()
            << EAM(aNumFPtsStr, "NPt", true,
                   "Number of ficticious pts, Def=1 (1:27pts, 2:175pts)")
            << EAM(mNSym, "NSym", true,
                   ", Non-symetric point generation, Def=false")
            << EAM(mNRand, "NRand", true,
                   ", Random point generation, Def=false")
            << EAM(mRedFacSup, "RedFac", true,
                   "Residual image reduction factor, Def=20")
            << EAM(mCorrCalib, "CorCal", true,
                   "Model residual camera calib, Def=true")
            << EAM(mCorrGlob, "CorGlob", true,
                   "Global modelling of CorCal, Def=true")
            << EAM(mCorrIma, "CorIma", true,
                   "Per image modelling of CorCal, Def=false")
            << EAM(mResPoly, "Deg", true,
                   "Degree of polyn to smooth residuals (used only if "
                   "CorCal=true), Def=2")
    ...
    ELISE_ASSERT((aNumFPtsStr.size() == 1) || (aNumFPtsStr.size() == 3) ||
                     (aNumFPtsStr.size() == 4),
                 "NPt requires 3 or 4 values");
    if (aNumFPtsStr.size() == 1)
        mN5Pts = true;
    else {
        mNumFPts.x =
            RequireFromString<double>(aNumFPtsStr[0], "Points along 1st axis");
        mNumFPts.y =
            RequireFromString<double>(aNumFPtsStr[1], "Points along 2nd axis");
        mNumFPts.z =
            RequireFromString<double>(aNumFPtsStr[2], "Points along 3rd axis");
    }
    if (aNumFPtsStr.size() == 4)
        mAddCDG = RequireFromString<double>(
            aNumFPtsStr[3], "Add the CDG per respective cple/tri");

    Initialize();

    if (mCorrCalib) CalcResidPoly();
...
void Test_AddVPts2Map() {
    ...
    for (tListM::const_iterator itM = aLM.begin(); itM != aLM.end(); itM++) {
        ...
        if ((*itM)->NbSom() == 3) {
            std::cout << "0 " << (*itM)->GetVal(0) << " 1 " << (*itM)->GetVal(1)
                      << " 2 " << (*itM)->GetVal(2) << "\n";
        }
    }
    getchar();
...
void cAppliFictObs::AddVPts2Map(tMapM& aMap, ElPackHomologue& aPack, int anInd1,
                                int anInd2) {
...
double cAppliFictObs::BsurH(ElSeg3D& aDir1, ElSeg3D& aDir2) {
    ...
    double aAngle = acos((aDN1.x * aDN2.x + aDN1.y * aDN2.y + aDN1.z * aDN2.z) /
                         (euclid(aDN1) * euclid(aDN2)));
    ...
    if (0) {
        ...
        Pt3dr aDN2t(-1, 0, 3);
        double aaaa =
            acos((aDN1t.x * aDN2t.x + aDN1t.y * aDN2t.y + aDN1t.z * aDN2t.z) /
                 (euclid(aDN1t) * euclid(aDN2t)));
        std::cout << "ang=" << aaaa * 180 / 3.14
                  << ", aBsH=" << 2 * tan(aaaa / 2) << "\n";
        getchar();
    }
...
bool cAppliFictObs::CalculateEllipseParam2(cXml_Elips3D& anEl,
                                           std::vector<const CamStenope*>& aVC,
                                           const std::string& aName1,
                                           const std::string& aName2,
                                           int& NbPts) {
    ELISE_ASSERT(
        int(aVC.size()) == 2,
        "cAppliFictObs::CalculateEllipseParam. Two cameras are required.");
    ...
    std::cout << " ElPackHomologue  dddddddhhhhhhhhhhhhhhhhhhhhhh " << aN << " "
    << aVC.at(0)->Focale() << " " <<  aVC.at(1)->Focale() << " " <<
    aVC.at(0)->VraiOpticalCenter() << " " << aVC.at(1)->VraiOpticalCenter()  <<
    "\n";
    */
    ...
    if (int(aPack.size()) > MinNbPtTri) {
        for (ElPackHomologue::const_iterator itP = aPack.begin();
            ...
            if (0)
                std::cout << "dddddddhhhhhhhhhhhhhhhhhhhhhh "
                          << aVSeg.at(0).P0() << " " << aVSeg.at(0).P1() << " "
                          << aVSeg.at(1).P0() << " " << aVSeg.at(1).P1() << " "
                          << aPt1 << " " << aPt2 << "\n";
            ...
            if (aResid < MaxReprojErr) {
                ...
                // std::cout << "aResid=" << aResid << ", aBsH=" << aBsH << " "
                // << aPds << "cpl\n";
...
bool cAppliFictObs::CalculateEllipseParam3(cXml_Elips3D& anEl,
                                           std::vector<const CamStenope*>& aVC,
                                           const std::string& aName1,
                                           const std::string& aName2,
                                           const std::string& aName3,
    ...
    ELISE_ASSERT(
        int(aVC.size()) == 3,
        "cAppliFictObs::CalculateEllipseParam. Three cameras are required.");
...
if (ELISE_fp::exist_file(aN23))
    ...
    if (int(aLM.size()) > MinNbPtTri) {
        for (tListM::const_iterator itM = aLM.begin(); itM != aLM.end();
             itM++) {
            std::vector<ElSeg3D> aVSeg;
            if ((*itM)->NbSom() == 3) {
                // std::cout << "=========== " << (*itM)->GetVal(0) << " "<<
                // (*itM)->GetVal(1) << " " << (*itM)->GetVal(2) << "\n";
                ...
                double aResid = 0;
                aResid +=
                    euclid(aVC.at(0)->Ter2Capteur(aInt) - (*itM)->GetVal(0));
                aResid +=
                    euclid(aVC.at(1)->Ter2Capteur(aInt) - (*itM)->GetVal(1));
                aResid +=
                    euclid(aVC.at(2)->Ter2Capteur(aInt) - (*itM)->GetVal(2));
                aResid /= 3;
                ...
                if (aResid < MaxReprojErr) {
                    ...
                    std::cout << "Residual " << aResid
                              << " which is bigger than " << MaxReprojErr
                              << "\n";
                }
            }
        }

        if (NbPts < MinNbPtTri) return false;
...
void cAppliFictObs::CalculteFromHomol2(std::vector<const CamStenope*>& aVC,
    ...
    for (ElPackHomologue::const_iterator itP = aPack.begin();
        ...
        if (0)
            std::cout << "dddddddhhhhhhhhhhhhhhhhhhhhhh " << aVSeg.at(0).P0()
                      << " " << aVSeg.at(0).P1() << " " << aVSeg.at(1).P0()
                      << " " << aVSeg.at(1).P1() << " " << aPt1 << " " << aPt2
                      << "\n";
...
bool cAppliFictObs::IsInSz(Pt2dr& aP) const {
    if ((aP.x > 0) && (aP.x < mSz.x) && (aP.y > 0) && (aP.y < mSz.y))
        return true;
    else
        return false;
}

bool cAppliFictObs::CalcRedFac(const std::vector<const CamStenope*>& aVC,
                               const std::vector<Pt3dr>& aVP, const Pt3dr& aCDG,
                               double& aRedOut) {
    /* back-proj the fict points to the triplet/cple
           and memorize the point that falls the farthest in aPOut3D & aPOutMax
       & aCamIdPOutMax
        */
...
void cAppliFictObs::FilterPtOutOfImg(const std::vector<const CamStenope*>& aVC,
                                     const Pt3dr& aPTer,
                                     const std::pair<int, TripleStr*>& aT,
                                     std::vector<int>& aTriIdsIn,
                                     std::vector<int>& aTriIdsOut,
                                     std::vector<Pt2dr>& aPImOut) {
    for (int aC = 0; aC < int(aVC.size()); aC++) {
        // back-project
        ...
        if (IsInSz(aPt)) {
            ...
            if (mCorrCalib) {
                if (mCorrIma)
                    ...
                    std::cout << "Something went wrong; check mCorrCalib, "
                                 "mCorrGlob and mCorrIma"
                              << "\n";
            }
...
void cAppliFictObs::GenerateFicticiousObsInEl(cGenGaus3D& aGG1,
...
void cAppliFictObs::GenerateFicticiousObs() {
    ...
    /********** Pour chaque triplet/cple recouper son elipse3d et genere les obs
     * fict ************/
    ...
    for (auto aT : mTriMap) {
        ...
        std::vector<const CamStenope*> aVC;
        if (aT.second->mC3)
            aVC = {aT.second->mC1, aT.second->mC2, aT.second->mC3};
        else
            aVC = {aT.second->mC1, aT.second->mC2};

        if (0) {
            std::cout << "C1=" << aT.second->mC1->Focale() << " "
                      << aT.second->mC1->PP() << " "
                      << aT.second->mC1->VraiOpticalCenter()
                      << "\n";  // aT.second->mC1->Dist()
            std::cout << "C2=" << aT.second->mC2->Focale() << " "
                      << aT.second->mC2->PP() << " "
                      << aT.second->mC2->VraiOpticalCenter()
                      << "\n";  // aT.second->mC2->Dist()
            if ((aT.second->mC3))
                std::cout << "C3=" << aT.second->mC3->Focale() << " "
                          << aT.second->mC3->PP() << " "
                          << aT.second->mC3->VraiOpticalCenter()
                          << "\n";  // aT.second->mC3->Dist()
            getchar();
        }
        ...
        if (aT.second->mC3)
            aTriIds = {aT.second->mId1, aT.second->mId2, aT.second->mId3};
        else
            aTriIds = {aT.second->mId1, aT.second->mId2};
        ...
        if (aT.second->mC3) {
            if (mCalcElip) {
                SUCCESS_ELLIPSE = CalculateEllipseParam3(
                    anEl, aVC, mSetName->at(aT.second->mId1),
                    mSetName->at(aT.second->mId2),
                    mSetName->at(aT.second->mId3), aNbPts3D);

                // original tie-pts are used
                if (!SUCCESS_ELLIPSE) {
                    CalculteFromHomol3(aVC, mSetName->at(aT.second->mId1),
                                       mSetName->at(aT.second->mId2),
                                       mSetName->at(aT.second->mId3), aVP);

                    aNbPts3D = int(aVP.size());

                    std::cout
                        << "ORIGINAL PTS FOR: " << mSetName->at(aT.second->mId1)
                        << " " << mSetName->at(aT.second->mId2) << " "
            ...
            // Read from orientation file (generated in Martini)
                ...
                std::string aName3R = mNM->NameOriOptimTriplet(
                    true, mSetName->at(aT.second->mId1),
                    mSetName->at(aT.second->mId2),
                    mSetName->at(aT.second->mId3));
                cXml_Ori3ImInit aXml3Ori =
                    StdGetFromSI(aName3R, Xml_Ori3ImInit);
                anEl = aXml3Ori.Elips();
        ...
        {
            if (mCalcElip) {
                SUCCESS_ELLIPSE = CalculateEllipseParam2(
                    anEl, aVC, mSetName->at(aT.second->mId1),
                    mSetName->at(aT.second->mId2), aNbPts3D);

                // original tie-pts are used
                if (!SUCCESS_ELLIPSE) {
                    CalculteFromHomol2(aVC, mSetName->at(aT.second->mId1),
                                       mSetName->at(aT.second->mId2), aVP);
                    aNbPts3D = int(aVP.size());
            ...
            // Read from orientation file (generated in Martini)
                ...
                std::string aNamOri =
                    mNM->NameXmlOri2Im(mSetName->at(aT.second->mId1),
        ...
        for (int aK = 0; aK < (int)aVP.size(); aK++) {
            ...
            if (aTriIdsIn.size() > 1) {
                ...
                std::cout << "a POint definitevely removed from a triple/cple "
                          << aVP.at(aK) << "\n";
            }
            ...
            if (0) {
                ...
                std::cout << " YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY "
                          << aVC.size() << " " << aPImIn.size();
                if (aTriIdsIn.size() > 1) {
                    for (int aC = 0; aC < int(aVC.size()); aC++) {
                        aSegV.push_back(
                            aVC.at(aC)->Capteur2RayTer(aPImIn.at(aC)));
                        aVPds.push_back(1.0);
                    }

                    bool ISOK = false;
                    Pt3dr aPVerif =
                        ElSeg3D::L2InterFaisceaux(&aVPds, aSegV, &ISOK);
        ...
        if (mPly) {
            ...
            std::string Ply1Dos =
                Ply0Dos + "NSym" + ToString(mNSym) + "_Pts-" +
                (mN5Pts ? "5Pts_"
                        : (ToString(mNumFPts.x) + ToString(mNumFPts.y) +
                           ToString(mNumFPts.z) + "_" + ToString(mAddCDG))) +
                "/";
            std::string Ply1File =
...
void cAppliFictObs::SaveHomolOne(std::vector<int>& aId,
        ...
        for (int aK1 = 0; aK1 < (int)aId.size(); aK1++) {
            for (int aK2 = 0; aK2 < (int)aId.size(); aK2++) {
                if (aK1 != aK2) {
                    ...
                    std::string aNameH = mNM->ICNM()->Assoc1To2(
                        "NKS-Assoc-CplIm2Hom@" + mOut + "@" + mHomExpOut, aN1,
                        aN2, true);

                    if (DicBoolFind(mHomRed, aNameH)) {
                        ElCplePtsHomologues aP(
                            aPImV.at(aK1), aPImV.at(aK2),
                            aAttr.at(0));  // a priori peut importe lequel K
                                           // pour l'attribute comme il sera
                                           // homogene par track ie par ellipse
...
void cAppliFictObs::UpdateAROne(const ElPackHomologue* aPack,
    ...
    if (aC1 && aC2) {
        for (ElPackHomologue::const_iterator itP = aPack->begin();
            ...
            double aRes1 =
                aC1->EpipolarEcart(itP->P1(), *aC2, itP->P2(), &aDir1);
            double aRes2 =
                aC2->EpipolarEcart(itP->P2(), *aC1, itP->P1(), &aDir2);

            if ((aRes1 < mResMax) && (aRes1 > -mResMax) && (aRes2 > -mResMax) &&
                (aRes2 < mResMax)) {
            ...
            /* else   //it still somewhat strange there are pts with this large
            spread
            {
...
void cAppliFictObs::UpdateAR(const ElPackHomologue* Pack12,
                             ...
                             const ElPackHomologue* Pack23, const int aTriId) {
...
void cAppliFictObs::CalcResidPoly() {
    ...
    if (mCorrIma) {
        for (auto aCam : mNameMap)
            mAR[aCam.second] =
                new cAccumResidu(mSz, mRedFacSup, OnlySign, mResPoly);
    }
    ...
    for (auto aT : mTriMap) {
        /* Cam1 - Cam2 Homol */
        const ElPackHomologue aElHom12 = mNM->PackOfName(
            mSetName->at(aT.second->mId1), mSetName->at(aT.second->mId2));

        /* Cam1 - Cam3 Homol */
        const ElPackHomologue aElHom13 = mNM->PackOfName(
            mSetName->at(aT.second->mId1), mSetName->at(aT.second->mId3));

        /* Cam2 - Cam3 Homol */
        const ElPackHomologue aElHom23 = mNM->PackOfName(
            mSetName->at(aT.second->mId2), mSetName->at(aT.second->mId3));
    ...
    FILE* aFileExpImRes =
        FopenNN("StatRes.txt", "w", "cAppliFictObs::CalcResidPoly");
    ...
    if (0)
        if (mCorrIma)
            for (auto aCam : mNameMap) {
                mAR[aCam.second]->Export("./",
                                         StdPrefix(aCam.first) + "-TestRes",
                                         aUEIR, aFileExpImRes);
            }
    if (0)
        if (mCorrGlob)
...
void cAppliFictObs::Initialize() {
    ...
    if ((mNbIm == 2) && (!EAMIsInit(&mResPoly))) mResPoly = 0;

    for (int aK = 0; aK < mNbIm; aK++) mNameMap[mSetName->at(aK)] = aK;
    ...
    if (DOTRI) {
        ...
        for (auto a3 : mLT.Triplets()) {
            ...
            if (DicBoolFind(mNameMap, a3.Name1()) &&
                ...
                std::string aName3R = mNM->NameOriOptimTriplet(
                    true, a3.Name1(), a3.Name2(), a3.Name3());
                cXml_Ori3ImInit aXml3Ori =
                    StdGetFromSI(aName3R, Xml_Ori3ImInit);
                ...
                mTriMap[aTriNb] = new TripleStr(aC1, mNameMap[a3.Name1()], aC2,
                                                mNameMap[a3.Name2()], aC3,
                                                mNameMap[a3.Name3()]);

                if (0)
                    std::cout << "Triplet " << aTriNb << " "
                              << mNameMap[a3.Name1()] << "-" << a3.Name1()
                              << " " << mNameMap[a3.Name2()] << "-"
                              << a3.Name2() << " " << mNameMap[a3.Name3()]
                              << "-" << a3.Name3() << "\n";
    ...
    if (DOCPLE) {
        ...
        for (auto a2 : mLCpl.Cple()) {
            ...
            if (DicBoolFind(mNameMap, a2.N1()) &&
                ...
                if (!OK)
                    std::cout << "cAppliFictObs::Initialize() warning - no "
                                 "elipse3D for couple "
                              << a2.N1() << " " << a2.N2() << "\n";
                ...
                // std::cout << "========================= C1 " << aC1->Focale()
                // << " " << aC1->PP() << " " << aC1->VraiOpticalCenter() << " "
                // << mNameMap[a2.N1()] << "\n"; std::cout <<
                // "========================= C2 " << aC2->Focale() << " " <<
                // aC2->PP() << " " << aC2->VraiOpticalCenter() << " " <<
                // mNameMap[a2.N2()] << " TriNb=" << aTriNb << "\n";

                mTriMap[aTriNb] = new TripleStr(aC1, mNameMap[a2.N1()], aC2,
                                                mNameMap[a2.N2()]);
                aTriNb++;
            }
        }
    ...
    if (aTriNb != 0) {
        ...
        ELISE_ASSERT(false,
                     "cAppliFictObs::Initialize no couples or triplets found");
...
void cAppliFictObs::InitNFHomOne(std::string& N1, std::string& N2) {
    std::string aNameH = mNM->ICNM()->Assoc1To2(
        "NKS-Assoc-CplIm2Hom@" + mOut + "@" + mHomExpOut, N1, N2, true);
...
ElRotation3D OriCam2On1_t(const cNewO_NameManager* aNM,
                          const std::string& aNOri1, const std::string& aNOri2,
                          bool& OK) {
    OK = false;
...
int CPP_XmlOriRel2OriAbs_main(int argc, char** argv) {
    ...
    ElInitArgMain(argc, argv, LArgMain() << EAMC(aPattern, "Pattern of images"),
                  LArgMain()
                      << EAM(aOut, "Out", true, "Output directory, Def=Test")
                      << aCMA.ArgCMA());
#if (ELISE_windows)
    ...
    std::string aCom = MM3dBinFile("TestLib NO_AllOri2Im ") + "\"" + aPattern +
                       "\"" + " ExpTxt=" + ToString(aCMA.mExpTxt) +
                       " SH=" + aCMA.mPrefHom +
                       " OriCalib=" + aCMA.mNameOriCalib;
    std::cout << "COM " << aCom << "\n";
    System(aCom);
    ...
    for (int aK = 0; aK < aNbIm; aK++) aNameMap[aSetName->at(aK)] = aK;

    aNM =
        new cNewO_NameManager("", aCMA.mPrefHom, true, aDir, aCMA.mNameOriCalib,
                              aCMA.mExpTxt ? "txt" : "dat");
    ...
    for (auto a3 : aLT.Triplets()) {
        ...
        if (DicBoolFind(aNameMap, a3.Name1()) &&
            ...
            std::string aName3R = aNM->NameOriOptimTriplet(
                true, a3.Name1(), a3.Name2(), a3.Name3());
            ...
            CamStenope* aC1 = aNM->CalibrationCamera(a3.Name1())
                                  ->Dupl();  // aNM->CamOfName(a3.Name1());
            CamStenope* aC2 = aNM->CalibrationCamera(a3.Name2())
                                  ->Dupl();  // aNM->CamOfName(a3.Name2());
            CamStenope* aC3 = aNM->CalibrationCamera(a3.Name3())
                                  ->Dupl();  // aNM->CamOfName(a3.Name3());
            ...
            std::string aNameOriOut1 = aNM->ICNM()->Assoc1To1(
                "NKS-Assoc-Im2Orient@-" + aOut, a3.Name1(), true);
            std::string aNameOriOut2 = aNM->ICNM()->Assoc1To1(
                "NKS-Assoc-Im2Orient@-" + aOut, a3.Name2(), true);
            std::string aNameOriOut3 = aNM->ICNM()->Assoc1To1(
                "NKS-Assoc-Im2Orient@-" + aOut, a3.Name3(), true);
    ...
    if (aNbIm == 2) {
        for (auto a2 : aLCpl.Cple()) {
            ...
            if (DicBoolFind(aNameMap, a2.N1()) &&
                ...
                if (1) {
                    std::cout << "eeee " << aP2.tr() << " " << a2.N1() << " "
                              << a2.N2() << "\n";
                }

                CamStenope* aC1 = aNM->CalibrationCamera(a2.N1())
                                      ->Dupl();  // aNM->CamOfName(a2.N1());
                CamStenope* aC2 = aNM->CalibrationCamera(a2.N2())
                                      ->Dupl();  // aNM->CamOfName(a2.N2());
                ...
                ELISE_fp::MkDirSvp("Ori-" + aOut + "/");
                std::string aNameOriOut1 = aNM->ICNM()->Assoc1To1(
                    "NKS-Assoc-Im2Orient@-" + aOut, a2.N1(), true);
                std::string aNameOriOut2 = aNM->ICNM()->Assoc1To1(
                    "NKS-Assoc-Im2Orient@-" + aOut, a2.N2(), true);
...
int CPP_Rot2MatEss_main(int argc, char** argv) {
    ...
    ElInitArgMain(
        argc, argv, LArgMain() << EAMC(aPattern, "Pattern of images"),
    ...
    aNM = new cNewO_NameManager("", "", true, aDir, aCMA.mNameOriCalib,
                                aCMA.mExpTxt ? "txt" : "dat");
    ...
    if (ELISE_fp::exist_file(aNameLCple)) {
        ...
        ELISE_ASSERT(false,
                     "CPP_Rot2MatEss_main no computed relative orientation in "
                     "the defined pattern!")
        return EXIT_FAILURE;
    }
    ...
    for (auto a2 : aLCpl.Cple()) {
        // verify that the pair images are in the pattern
        if (DicBoolFind(aNameMap, a2.N1()) && DicBoolFind(aNameMap, a2.N2())) {
            // poses
            bool OK;
            ElRotation3D aP1 = ElRotation3D::Id;
            ...
            std::string aNameOut = aDir + aOut + "-" + StdPrefix(a2.N1()) +
                                   "_" + StdPrefix(a2.N2()) + ".xml";
            ...
            if (VERIF) {
                ...
                // ElRotation3D aRotVerif  =
                // NEW_MatEss2Rot(aMatEss,(aQuick?aPack30:aPack150));
...
class cAppliMinim : public cCommonMartiniAppli {
   public:
    ...
    ElRotation3D OptimalSol(std::vector<ElMatrix<double>>& aMat,
                            std::vector<ElSeg3D>& aDirs);
   ...
   private:
    ...
    ElRotation3D OriCam2On1(const std::string& aNOri1,
                            const std::string& aNOri2, bool& OK) const;
...
cAppliMinim::cAppliMinim(int argc, char** argv) : mOut("AbsPose") {
    std::string aDir;
    ...
    ElInitArgMain(
        ...
        LArgMain() << EAMC(mQIm, "The query image")
                   << EAMC(
                          aPatInOri,
                          "Pattern of image with known (absolute) orientation)")
                   << EAMC(mInOri, "(Absolute) orientation directory"),
        LArgMain()
            << EAM(mOut, "Out", true,
                   "Output directory, Def=Test")  // ???????????????????????????????????????????
            << aCMA.ArgCMA());
#if (ELISE_windows)
    ...
    // update the lists of couples and triplets
    std::string aCom = MM3dBinFile("TestLib NO_AllOri2Im ") + "\"" + aPatInOri +
                       "|" + mQIm + "\"" + " ExpTxt=" + ToString(aCMA.mExpTxt) +
                       " Quick=" + ToString(aCMA.mQuick);
    ...
    for (int aK = 0; aK < aNbIm; aK++) mNameMap[mSetName->at(aK)] = aK;

    mNM = new cNewO_NameManager("", "", aCMA.mQuick, aDir, aCMA.mNameOriCalib,
                                aCMA.mExpTxt ? "txt" : "dat", mOut);
    ...
    for (auto a2 : mLCpl.Cple()) {
        if ((DicBoolFind(mNameMap, a2.N1()) &&
            ...
            bool OK;
            mCplMap[a2.N1() + a2.N2()] =
                new ElRotation3D(OriCam2On1(a2.N1(), a2.N2(), OK));
            // mCplMap[a2.N1() + a2.N2()] = new
            // ElRotation3D(TestOriConvention(mInOri,a2.N1(),a2.N2()));
        }
    }
...
ElRotation3D cAppliMinim::OriCam2On1(const std::string& aNOri1,
                                     const std::string& aNOri2,
                                     bool& OK) const {
    OK = false;
...
/*
    Calculate the absolute pose of a query image given at least two connected
   absolute poses (i.e. reference images)
*/
...
void cAppliMinim::DoCalc() {
    ...
    for (auto a2 : mMapInOri) {
        ...
        if (DicBoolFind(mCplMap, a2.first + mQIm)) {
            ...
            std::cout << "No relative orientation for " << a2.first << "-"
                      << mQIm << "\n";
        }
        ...
        if (FOUNDREL) {
            ...
            if (0)
                std::cout << "P2 Rot\n"
                          << aP2.Mat()(0, 0) << " " << aP2.Mat()(0, 1) << " "
                          << aP2.Mat()(0, 2) << "\n"
                          << aP2.Mat()(1, 0) << " " << aP2.Mat()(1, 1) << " "
                          << aP2.Mat()(1, 2) << "\n"
                          << aP2.Mat()(2, 0) << " " << aP2.Mat()(2, 1) << " "
                          << aP2.Mat()(2, 2) << "\n";
    ...
    for (int aR = 0; aR < int(aVRQIm.size()); aR++) {
        ...
        if (0)
            std::cout << "Rot of the query image\n"
                      << aVRQIm.at(aR)(0, 0) << " " << aVRQIm.at(aR)(0, 1)
                      << " " << aVRQIm.at(aR)(0, 2) << "\n"
                      << aVRQIm.at(aR)(1, 0) << " " << aVRQIm.at(aR)(1, 1)
                      << " " << aVRQIm.at(aR)(1, 2) << "\n"
                      << aVRQIm.at(aR)(2, 0) << " " << aVRQIm.at(aR)(2, 1)
                      << " " << aVRQIm.at(aR)(2, 2) << "\n";
    }
    ...
    if (int(aVSeg.size()) >= 2) {
        ...
        aCQIm->SetOrientation(
            ElRotation3D(aRes.inv().tr(), aRes.inv().Mat(), true));
        ...
        if (0) {
            ...
            for (int aK = 0; aK < int(aVSeg.size()); aK++) {
                ...
                if (aDiff < aDifInf) aDifInf = aDiff;
            }
            // new weights
        ...
        std::cout
            << "Not enough 3D lines to rocover the pose of the query image"
            << "\n";
}

ElRotation3D cAppliMinim::OptimalSol(std::vector<ElMatrix<double>>& aMat,
                                     std::vector<ElSeg3D>& aDirs) {
...
ElRotation3D cAppliMinim::TestOriConvention(std::string& aNameOri,
                                            const std::string& aNameIm1,
                                            const std::string& aNameIm2) const {
    std::cout << "InOri=" << aNameOri << ", Im1: " << aNameIm1
              << ", Im2: " << aNameIm2 << "\n";
    ...
    std::cout << "  +ElRot rel \n"
              << aRot.Mat()(0, 0) << " " << aRot.Mat()(0, 1) << " "
              << aRot.Mat()(0, 2) << "\n"
              << aRot.Mat()(1, 0) << " " << aRot.Mat()(1, 1) << " "
              << aRot.Mat()(1, 2) << "\n"
              << aRot.Mat()(2, 0) << " " << aRot.Mat()(2, 1) << " "
              << aRot.Mat()(2, 2) << "\n";
    std::cout << "  +ElTr rel : " << aRot.tr().x << " " << aRot.tr().y << " "
              << aRot.tr().z << "\n";
