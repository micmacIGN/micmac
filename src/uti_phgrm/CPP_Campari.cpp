...
int Campari_main(int argc,char ** argv)
{
    ...
    LArgMain()  << EAM(GCP,"GCP",false,"[GrMes.xml,GrUncertainty,ImMes.xml,ImUnc]")
                    << EAM(EmGPS,"EmGPS",false,"Embedded GPS [Gps-Dir,GpsUnc]")
                    << EAM(aSigmaTieP,"SigmaTieP","Sigma use for TieP weighting (Def=1)")
                    << EAM(aFactResElimTieP,"FactElimTieP","Fact elimination of tie point (prop to SigmaTieP, Def=5)")
