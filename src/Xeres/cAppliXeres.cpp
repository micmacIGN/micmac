...
cXeres_Cam::cXeres_Cam(const std::string & anId,cAppliXeres & anAppli) :
   mAppli   (anAppli),
   mId      (anId),
...
double & cXeres_Cam::TmpDist() {return mTmpDist;}
const cXeres_NumSom & cXeres_Cam::NS() const { return mNS; }
const bool & cXeres_Cam::HasIm() const { return mHasIm; }
const std::string & cXeres_Cam::NameIm() const {return mNameIm;}
    ...
    mFilter       (aFilter)
{
    ...
    {
         for (int anI = 1; anI <= 7 ; anI++)
         {
              AddCam(std::string(1,aC) + ToString(anI));
         }
    }
...
void cAppliXeres::AddCam(const  std::string & anId)
{
   mVCam.push_back(new cXeres_Cam(anId,*this));
   mMapCam[anId] = mVCam.back();
}

std::string cAppliXeres::NameOfId(const std::string & anId)
{
