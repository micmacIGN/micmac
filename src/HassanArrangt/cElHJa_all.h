...
class cElHJaAttrSomPlani
{
    public :
        cElHJaAttrSomPlani();
        cElHJaAttrSomPlani(Pt2dr,REAL anACurvE);
        Pt2dr Pt() const;
        REAL  ACurvE() const;
        REAL  ACurvD() const;
        void SetCurvD(REAL);
...
class cElHJaAttrArcPlani
{
	...
	void SetArcHom(tArcGrPl *);
	void SetFacette(cElHJaFacette *);
	INT  Cpt() const;
	void ResetCpt();
	void Set1();
	void IncrCpt();
...
class cFullSubGrWithP : public tFullSubGrPl
{
    public :
	cFullSubGrWithP () ;
	cFullSubGrWithP (Seg2d aSeg) ;

        Pt2dr pt(tSomGrPl & aSom);
	REAL pds(TArc & anArc);
