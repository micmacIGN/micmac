...
int InitOriLinear_main(int argc,char ** argv)
{
    ...
    std::string aFullPatternNewImages, aFullPatternRefImages, aOriRef;//pattern of all files
    ...
    LArgMain()  << EAMC(aFullPatternNewImages, "Pattern of images to orientate",  eSAM_IsPatFile)
				...
				<< EAMC(aFullPatternRefImages, "Pattern of already-oriented images, used to compute orientation and movement",  eSAM_IsPatFile),
    //optional arguments
    LArgMain()  
    );
