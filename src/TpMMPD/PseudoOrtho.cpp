...
int PseudoOrtho_main(int argc, char **argv) {
    ...
    double dRegul=0.5; //Un point est plus au sol qu'un autre si l'écart en Z est au moins dRegul
    Box2dr aBoxTerrain;
    ...
    (
        ...
        LArgMain()  << EAM(resolutionPlani,"Resol",true,"output ortho resolution")
                    << EAM(dRegul,"Regul",true,"Regularisation parameter")
     );
    ...
    // Creation de l'ortho
    Pt2di SzOrtho(NC,NL);
    TIm2D<U_INT1,INT4>* ptrOrtho = new TIm2D<U_INT1,INT4>(SzOrtho);
        ...
        for(int c=0;c<NC;++c)
        {
            ...
            for(size_t i=0;i<aVNuages.size();++i)
            {
                ...
                if (aVNuages[i]->CaptHasData(aPCapteur))
                {
                    ...
                    double d = aP.z - P3d.z;
                    double d3d = sqrt(pow(aP.x - P3d.x,2) + pow(aP.y - P3d.y,2) + pow(aP.z - P3d.z,2));
                    if (verbose)
                        if (P3d.x==954447 && P3d.y==6228966)
                            std::cout << "diff pour le nuage "<<i<<" : "<<d<<std::endl;

                    if ((best_image == -1)||(dmin>d3d+dRegul))
                    {
                        best_image = i;
                        dmin = d3d;
                        best_Pt = aPIm*aVNuages[0]->ResolImRefFromCapteur();
                    }
                }
                //else
    ...
    Tiff_Im out(aNameResult.c_str(), ptrOrtho->sz(),GenIm::u_int1,Tiff_Im::No_Compr,Tiff_Im::BlackIsZero);
