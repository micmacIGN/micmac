...
//----------------------------------------------------------------------------
int ConvHomolVSFM2MM_main(int argc,char ** argv)
{
    if ( !g_externalToolHandler.get( "Conv_VSFM_MM" ).isCallable())
    {
        ELISE_ERROR_RETURN("cannot find Conv_VSFM_MM tool");
    }
    else
    {
        cout<<"Conv_VSFM_MM tool founded ! "<<endl;
        string aPat = "";
        string aOut = "Homol_VSFM";
        ElInitArgMain
            (
            argc,argv,
            LArgMain()  << EAMC(aPat,"Pattern", eSAM_IsPatFile),
            LArgMain()
                        << EAM(aOut,"Out",false,"Out homol - def=Homol_VSFM")
            );

        #ifdef linux
                string aCom_RegEx = MM3DStr + " TestRegEx " + aPat + " ExpList=lstImg";

                cout << "\n**********************Create list Image**********************" << endl;
                system_call(aCom_RegEx.c_str());


                string aCom_Conv = g_externalToolHandler.get( "Conv_VSFM_MM" ).callName()
                        + " lstImg "
                        + aOut;

            cout << "\n********************** Running Conversion **********************" << endl;

            system_call(aCom_Conv.c_str());
        #endif
