...
//GERALD
#if (ELISE_windows)
std::string TheEliseDirXmlSpec="..\\include\\XML_GEN\\";		
#else
std::string TheEliseDirXmlSpec="include/XML_GEN/";			
#endif
...
std::string StdGetFileXMLSpec(const std::string & aName)
{
	...
	return TheEliseDirXmlSpec+ aName;
}
...
cElXMLTree::cElXMLTree(const std::string & aName,cArgCreatXLMTree * anArgEx,bool DoFileInclu) 
{
	...
	while(1)
	{
		...
		{
			// std::cout << "_____________NAME = " << aName << "\n";
			VerifCreation();
			delete aFp;
			return;
		}
...
{
	mValTag = aTok.Val();
	// std::cout << "XXXXX-mt: " << anArg.ModifTree() << " " << mValTag << "\n";
	mPere = aPere;
	if (aTok.Kind() == eXMLStd)
	{
		// if (aUseSubstTree) std::cout << "============   " << mValTag << "\n";
		if (aUseSubstTree)
			anArg.DoSubst(mValTag);
		mKind = eXMLFeuille;
		// std::cout << "_____________NAME  " << aFp->Name() << "\n";
		VerifCreation();
		return;
	}
	mKind = eXMLBranche;
	...
	{	   
		...
		if (aNewTok.Kind() == eXMLClose)
		{
			if (aNewTok.Val() != aTok.Val())
			{
				...
				ELISE_ASSERT
					(
					false,
					"bad Close Tag in cElXMLTree::cElXMLTree"
					);
			}
			VerifCreation();
			{
			...
			)
		{
				...
				)
			{
				...
				// std::cout << "OUT = " << aNF <<  " MT " << anArg.ModifTree() << "\n";
				// LALA
				// cElXMLTree aPetitFils((*it)->mValTag);
				cElXMLTree aPetitFils(aNF,&anArg);
				for 
					(
					std::list<cElXMLTree *>::iterator it2=aPetitFils.mFils.begin(); 
				it2!=aPetitFils.mFils.end() ; 
				it2++
					)
				{
					mFils.push_back(*it2);
