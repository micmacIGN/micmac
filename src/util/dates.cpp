...
cCameraEntry *  CamOfName(const std::string & aName)
{
	...
	{
		statTheDicCam = new std::map<std::string,cCameraEntry>;
		DC_Add(statTheDicCam,MMDir()+"include/XML_User/DicoCamera.xml");
		DC_Add(statTheDicCam,MMDir()+"include/XML_MicMac/DicoCamera.xml");
	}
	...
	*/
	{
		...
		{
			aModeDCRaw  = (aKTest==0);
			std::string aStrCom = aModeDCRaw ? (STRLANG+MMDir()+"bin/ElDcraw -i -v ") : (STRLANG+ " exiv2 ");
