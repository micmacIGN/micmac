...
)
{
    ...
    Len =  static_cast<int *>(amd_malloc (n * sizeof (Int))) ;
    Pinv = static_cast<int *>(amd_malloc (n * sizeof (Int))) ;
    mem += n ;
    mem += n ;
    if (!Len || !Pinv)
    {
	...
	Rp = static_cast<int *>(amd_malloc ((n+1) * sizeof (Int))) ;
	Ri = static_cast<int *>(amd_malloc (MAX (nz,1) * sizeof (Int))) ;
	mem += (n+1) ;
	mem += MAX (nz,1) ;
	if (!Rp || !Ri)
	{
    ...
    if (ok)
    {
	S = static_cast<int *>(amd_malloc (slen * sizeof (Int))) ;
    }
    AMD_DEBUG1 (("slen %g\n", (double) slen)) ;
    if (!S)
    {
