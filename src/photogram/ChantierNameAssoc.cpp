...
void MMD_InitArgcArgv(int argc,char ** argv,int aNbMin)
{
	...
	if ((ArgvMMDir=="") && (argc!=0))
	{
		...
		*/
		...
		{
			cElRegex  anAutomProg("(.*)bin/"+aPatProg,10);
...
namespace NS_ParamChantierPhotogram{
	...
	*/
	...
	void cMultiNC::AutoMakeSubDirRec(const tNuplet & aNuplet)
	{
		if (mSubDirRec) 
		{
			// std::cout << "ASDRASDR : " << mDir+aNuplet[0] <<  " " << this <<  "\n";
...
aKeyOrFile         :
		...
		)
	{
		...
		// if (ELISE_fp::exist_file(aFullNG))
	...
	void DoSimplePastisSsResol(const std::string & aFullName,int aResol)
	{
		...
		// std::cout << "AVANT "   << aScale  << " APRES " << Arrondi/iScale << "\n";
		...
		*/
		...
		//GERALD
			...
			std::string PastisFolder = "Pastis\\";
		...
		{
			ELISE_fp::MkDirRec(aDir+PastisFolder);
		...
		mRTr_R2I   (Pt2dr(0,0),Pt2dr(1,0))
	{
		...
		if ((mScale !=1.0) || (mTeta!=0.0) ||  aWithBox)
		{
			...
			ELISE_fp::MkDir(aDir+PastisFolder);
		...
		)
	{
		...
		std::string aDir = mDir + aManquant + anOri + "/";
