...
static edge_node **bound_list(lmt_node **lmt, double y)
{
  ...
  if (!*lmt)
  {
    ...
    void * vd;
    MALLOC(vd, sizeof(lmt_node), "LMT insertion");
    *lmt = (lmt_node *)vd;
...
static void add_to_sbtree(int *entries, sb_tree **sbtree, double y)
{
  if (!*sbtree)
  {
    ...
    void * vd;
    MALLOC(vd, sizeof(sb_tree), "scanbeam tree insertion");
    *sbtree = (sb_tree*) vd;
    (*sbtree)->y= y;
    (*sbtree)->less= NULL;
    (*sbtree)->more= NULL;
                            ...
                            gpc_op op)
{
  ...
  void * vd;
  MALLOC(vd, total_vertices * sizeof(edge_node),
         "edge table creation");
  edge_table = (edge_node*) vd;
                             ...
                             double x, double y)
{
  ...
  if (!*it)
  {
    ...
    void * vd;
    MALLOC(vd, sizeof(it_node), "IT insertion");
    *it = (it_node*) vd;
                        ...
                        double dy)
{
  ...
  if (!*st)
  {
    ...
    void * vd;
    MALLOC(vd, sizeof(st_node), "ST insertion");
    *st = (st_node *) vd;
...
static void add_left(polygon_node *p, double x, double y)
{
  ...
  void * vd;
  MALLOC(vd, sizeof(vertex_node), "vertex node creation");
  nv = ( vertex_node * ) vd;
                          ...
                          double x, double y)
{
  ...
  void* vd;
  MALLOC(vd, sizeof(polygon_node), "polygon node creation");
  *p = ( polygon_node * ) vd;

  /* Create a new vertex node and set its fields */
  void* vd2;
  MALLOC(vd2, sizeof(vertex_node), "vertex node creation");
  nv = ( vertex_node* ) vd2;
...
static void add_vertex(vertex_node **t, double x, double y)
{
  ...
  {
    void * vd;
    MALLOC(vd, sizeof(vertex_node), "tristrip vertex creation");
    *t = ( vertex_node * ) vd;
                         ...
                         double x, double y)
{
  ...
  {
    void * vd;
    MALLOC(vd, sizeof(polygon_node), "tristrip node creation");
    *tn = ( polygon_node * ) vd;
    (*tn)->next= NULL;
    (*tn)->v[LEFT]= NULL;
    (*tn)->v[RIGHT]= NULL;
...
static bbox *create_contour_bboxes(gpc_polygon *p)
{
  ...
  int   c, v;
  void * vd;
  MALLOC(vd, p->num_contours * sizeof(bbox), "Bounding box creation");
  box = (bbox*) vd;
...
static void minimax_test(gpc_polygon *subj, gpc_polygon *clip, gpc_op op)
{
  ...
  void * vd;
  MALLOC(vd, subj->num_contours * clip->num_contours * sizeof(int),
...
void gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
  ...
  VoidFscanf(fp, "%d", &(p->num_contours));
  void * vd;
  MALLOC(vd, p->num_contours * sizeof(int),
         "hole flag array creation");
  p->hole = ( int* ) vd;

  MALLOC(vd, p->num_contours
         * sizeof(gpc_vertex_list), "contour creation");
  p->contour = (gpc_vertex_list*) vd;

  for (c= 0; c < p->num_contours; c++)
  {
    VoidFscanf(fp, "%d", &(p->contour[c].num_vertices));

    if (read_hole_flags)
      VoidFscanf(fp, "%d", &(p->hole[c]));
    else
      p->hole[c]= FALSE; /* Assume all contours to be external */

    MALLOC(vd, p->contour[c].num_vertices
           * sizeof(gpc_vertex), "vertex creation");
    p->contour[c].vertex = (gpc_vertex*) vd;

    for (v= 0; v < p->contour[c].num_vertices; v++)
      VoidFscanf(fp, "%lf %lf", &(p->contour[c].vertex[v].x),
...
void gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
  ...
  void * vd;
  MALLOC(vd, (p->num_contours + 1)
         * sizeof(int), "contour hole addition");
  extended_hole = (int*) vd;

  /* Create an extended contour array */
  MALLOC(vd, (p->num_contours + 1)
         * sizeof(gpc_vertex_list), "contour addition");
  extended_contour = ( gpc_vertex_list *) vd;
  ...
  MALLOC(vd, new_contour->num_vertices
         * sizeof(gpc_vertex), "contour addition");
  extended_contour[c].vertex = ( gpc_vertex* ) vd;
                      ...
                      gpc_polygon *result)
{
  ...
  void* vd;
  MALLOC(vd, sbt_entries * sizeof(double), "sbt creation");
  sbt = ( double * ) vd;
  ...
  {
    void * vd;
    MALLOC(vd, result->num_contours
           * sizeof(int), "hole flag table creation");
    result->hole = ( int*) vd;

    MALLOC(vd, result->num_contours
           * sizeof(gpc_vertex_list), "contour creation");
    result->contour = (gpc_vertex_list*) vd;
    ...
    for (poly= out_poly; poly; poly= npoly)
    {
      ...
      if (poly->active)
      {
        ...
        MALLOC(vd,
          ...
          "vertex creation");
        result->contour[c].vertex = ( gpc_vertex* ) vd;
                       ...
                       gpc_tristrip *result)
{
  ...
  {
    void * vd;
    MALLOC(vd, result->num_strips * sizeof(gpc_vertex_list),
           "tristrip list creation");
    result->strip = ( gpc_vertex_list* ) vd;
    ...
    for (tn= tlist; tn; tn= tnn)
    {
      ...
      if (tn->active > 2)
      {
        ...
        MALLOC(vd, tn->active * sizeof(gpc_vertex),
               "tristrip creation");
        result->strip[s].vertex = ( gpc_vertex* ) vd;
        v= 0;
        if (INVERT_TRISTRIPS)
        {
          lt= tn->v[RIGHT];
