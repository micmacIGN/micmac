					...
					)
{
	...
	for (int aK =0; aK<vTriangles.size();++aK)
	{
		vector <Pt3dr> aPts;
		vector <float> attr;
		vTriangles[aK].getVertexes(aMesh, aPts);
		//aTri.getAttributes();
		ELISE_ASSERT(aPts.size() == 3, "cZBuffer.cpp Basculer(cMesh, float) aPts.size() != 3");
		
		BasculerUnTriangle(aPts[0], aPts[1], aPts[2], attr);
	}
...
void cZBuffer::BasculerUnTriangle(Pt3dr A,Pt3dr B,Pt3dr C,vector <float> const &attributes)
{
     Pt3dr A3  =  ProjDisc(A);
     Pt3dr B3  =  ProjDisc(B);
     Pt3dr C3  =  ProjDisc(C);
