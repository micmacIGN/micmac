...
\begin{itemize}
    ...
    \item {\tt TopoAdj} (see \ref{subsec:TopoAdj})
\end{itemize}

The topo measurements file can be given as a set of \CdPPP\ json or xml file, or in a simplified text format (named {\tt OBS} file) inherited from IGN's
...
An example of topo  adjustment can be found in \texttt{MMVII/MMVII-UseCaseDataSet/TopoMini/}.
...
Where PtA and PtD have known coordinates, PtB and PtC have to be determined (only approximate coordinates are known).
...
To do that, their \texttt{\_\_Opt\_\_Sigma2} nodes have to be deleted.
...
\subsection{Computation SysCo}

A \texttt{RTL} SysCo is mandatory to be able to make a topo compensation.
We can choose the initial coordinates of PtA as the tangence point.
The SysCo definition according to ~\ref{subsec:SysCoDef} is: \texttt{RTL*657700*6860700*0*IGNF:LAMB93}.

The \texttt{GCPChSysCo} command does the conversion:
...
\begin{itemize}
   \item an instrument positionned on PtA measures hz angle, zen angle and distance to PtB and PtC
   \item an instrument positionned on PtD measures hz angle, zen angle and distance to PtB and PtC
\end{itemize}
...
\end{verbatim}
This file has to be copied into a new directory in \texttt{MMVII-PhgrProj/Topo}:
\begin{lstlisting}
...
The number of unknowns in this adjustement is:
\begin{itemize}
   \item 3 per point ($x, y, z$)
   \item 1 per station ($G_0$)
\end{itemize}
...
\begin{itemize}
   \item 3 per constrained point, PtA and PtD
   \item 1 per topo measurement
\end{itemize}
...
Here the input topo directory is \texttt{Obs1} and the input PointsMeasure is \texttt{Init}.
We give output directories for topo and points, and add a weight factor for 3D constraintes coordinates,
to say that PtA and PtD are known with a precision of 1 mm:
\begin{lstlisting}
...
The last step is to convert the RTL coordinates in Lambert 93:
...
It is possible to say that the station on PtA was not verticalized:
\begin{verbatim}
#BASC
 7   PtA    PtB     0      0.001
 6   PtA    PtB   100      0.001
...
It will add two rotations unknowns to the computation.

It is also possible to remove the $G_0$ unknown of a station, expressing the fact that it
...
\begin{itemize}
   ...
   \item use a code 7 as first measurement
\end{itemize}
...
Minimal external constraints for the quickstart example:
having constraints on only one point and this \texttt{OBS} file:
