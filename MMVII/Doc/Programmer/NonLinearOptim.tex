...
\section{Topometric compensation example}

\subsection{Global presentation}

The beginning of a topometric compensation system can be found in \texttt{MMVII/src/Topo/}.

All computation happens in a 3D cartesian frame. For now, there is no georeferencing
and no Earth model.
...
\subsection{\texttt{TopoComp} example}

The example is created by the method \texttt{cTopoComp::createEx1()}.

\subsubsection{Step 1}

It starts with 3 fixed points ($A$, $B$, $C$), forming an isosceles triangle
...
The points have to be added as pointers to \texttt{cTopoComp::allPts()} since there
is no copy constructor for classes that derive from \texttt{cObjWithUnkowns}.
...
The point $D$ is initialized above the triangle, and distances from $A$, $B$ and $C$ to $D$
...
\caption{Point determined by measured distances}
\label{fig:topoEx2}
\end{figure}

Observations must refer to an object from a class derived from \texttt{cTopoObsSet},
used to attach parameters to observations.
In the case of measured distances there is no parameter, therefore, the \texttt{cTopoObsSetSimple}
class is used. These object must be created with \texttt{make\_TopoObsSet} method template.

The observations are all from the class \texttt{cTopoObs}, the observation type is given
to the constructor via an \texttt{TopoObsType}.
...
There are now 3 unknonws ($D$ coordinates) and 4 observations.

\subsubsection{Step 3}

The point $E$ is initialized inside the tetrahedron, and we add observations saying that
the distances from $E$ to $A$, $B$, $C$ and $D$ are equal but unkonwn (Fig. \ref{fig:topoEx3}).

\begin{figure}[!h]
\centering
\includegraphics[width=12cm]{Programmer/benchtopo3.png}
\caption{Point determined by common distance}
\label{fig:topoEx3}
\end{figure}

A new \texttt{cTopoObsSet} must be created. The derived \texttt{cTopoObsSetDistParam} type
has a parameter that will be shared between all the observations that belongs to it
...
This added 4 unknonws ($E$ coordinates and the common distance) and 4 observations.
...
After \texttt{cTopoComp} object creation and filling (see Bench example),
