...
%%:w void  AddFixVarTmp (int aNum,const Type& aVal,const Type& aWeight);

%     cSetIORSNL_SameTmp(const tStdVect & aValTmpUk,const tVectInd & aVFix={},const tStdVect & aValFix ={}); OWU
...
\begin{itemize}
     ...
     \item this object must describe their unknown as in intervall of {\tt double*};
...
The class organize the "coordination" between all object that will participate to the same solver.
...
\begin{itemize}
   ...
   \item    {\tt OnUpdate()}, this virtual call back, will be used after the  object has been
            modified by a {\tt SetVUnKnowns}.
\end{itemize}

SetFrozenVar % #OWU

%---------------------------------------------
\subsection{Example in bundle adjustment with camera classes}
...
The code corresponing can be found in :
...
\begin{itemize}
   ...
   \item {\tt cConvCalib.cpp}  that contain the  example where a bundle adjsutemnt is done;
         it's a very specific case, as the bundle is used to make a conversion of camera using
         generated $3d-2d$ correspondances;
