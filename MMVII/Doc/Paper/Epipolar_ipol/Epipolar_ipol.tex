...
The epipolar geometry of images plays a central role in many applications in the field of photogrammetry and computer vision. In the stereo-reconstruction pipeline, it is \ermpdok{used} twice:
...
We demonstrate that on a range of models, including Pleiades images (pushbroom), the Corona images (panoramic pushbroom), a consumer grade camera images (central perspective) as well as \ermpdok{Sentinel-1 radar images (pushbroom)}.
...
to $\pi_k^{-1}(p_k)$. \mpd{When there is no ambiguity},
...
can provide an elegant solution by separating the problem in two. \er{ER thinks this should be removed $\rightarrow$ When two images are in epipolar geometry, two points $p_1,p_2$ are potential homologous iff they have the same ordinate ($y_1=y_2$).}  \mpd{A discuter}
...
Analogously to the proof in Section~\ref{ExistEpip}, we will make a  computation of two-way paths, \ermpdok{$\BundO$ then $\BundT$,
as well as $\BundT$} \mpd{then  \sout{and}} $\BundO$. Then, we express the \ermpdok{intersection distance}
between these two paths as the Taylor expansion \footnote{ \mpd{On dirait pas plutot the Taylor expansion 
of the intersection distance}}  . Let's consider the following (see Figure~\ref{EqDifEpip}):
\begin{itemize}
   \item we make the quasi-vertical assumption\footnote{when this assumption can \mpd{not ?} be made,
...
degenerate case for stereovision} and we must then \ermpdok{make of $\delta'_2 -\delta_2$ and $\delta'_1 -\delta_1$ term of degre $2$:}
...
are  lines $\LineK(v')$ in $E_k$, one can observe that $\phi'_1 \phi_1^{-1}$  and $\phi'_2 \phi_2^{-1}$ are diffeomorphisms transforming lines into lines, and they globally \er{\sout{operate the same transformation on lines}}\mpd{Not sure to undestand}
...
inside each line \mpd{?\sout{of each \er{function}}}. For instance, one can impose that the colums remain constant (i.e.
...
In this paper, the examples processed by our method are pushbroom satellite models known by their RPCs, the central perspective and \ermpdok{radar} models. However, the only restriction imposed on the generic nature of the model is that the projection function is "smooth", i.e.:
...
To begin with, the method estimates the centers \ermpdok{\sout{[inconsistent with small c in the implemention]} $C_1,C_2$} of a set of points $p_1$ and $p_2$. This is done by calculating the average of all points' coordinates.  Then, the computations continue in
