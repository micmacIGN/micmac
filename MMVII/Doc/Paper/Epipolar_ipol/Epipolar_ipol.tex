...
unwanted high frequency behavior. In our case, this problem should never arise as the measurements  are synthesized from the projection functions $\pi_1,\pi_2$, which provides sufficient redundancy (for instance,
 hundreds of times more measurements than constraints). \mpd{Note, this will be a different issue when we use
...
if we have point correspondences between the image pairs but we don't know the geometric model?"} There is \mpd{NO}
a straightforward answer to whether it is possible or not. As elaborated in the following pros and cons arguments, the answer depends on the point correspondences, and is a trade-off between the nature of the 3D scene and the complexity of the model. 
