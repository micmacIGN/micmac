...
The epipolar geometry of images plays a central role in many applications in the field of photogrammetry and computer vision. In the stereo-reconstruction pipeline, it is \er{used} twice:
...
%
Unlike the central projection camera model, pushbroom-like sensors acquire each image row from a different perspective center. As a consequence, the epipolar lines are neither straight lines, nor are they conjugate across the image~\cite{Gupta1997}.  One way to overcome this particularity is to simplify the projection function with a 2D affine~\cite{ono1999epipolar,wang2011epipolar} or a parallel projection model~\cite{morgan2006epipolar}. Such approximations usually come at the price of precision, especially with the increasing camera field-of-view or in mountainous scenes. In the context of dense image matching, de~Francis \textit{et al.}~\cite{deFrancis2014stereo} improves the precision by partitioning the images into small patches, for which indepedent affine rectifictions are computed. Alternatively, and with equally good precision, Oh~\cite{Oh2011} uses the \textit{Rational Polynomial Coefficients} (RPCs) to map the epipolar curves across the full size images with lines in a piecewise approach.
%
...
We demonstrate that on a range of models, including Pleiades images (pushbroom), the Corona images (panoramic pushbroom), a consumer grade camera images (central perspective) as well as \er{Sentinel-1 radar images (pushbroom)}.
...
can provide an elegant solution by separating the problem in two. \er{ER thinks this should be removed $\rightarrow$ When two images are in epipolar geometry, two points $p_1,p_2$ are potential homologous iff they have the same ordinate ($y_1=y_2$).}
...
Analogously to the proof in Section~\ref{ExistEpip}, we will make a  computation of two-way paths, \er{$\BundO$ then $\BundT$,
as well as $\BundT$ and $\BundO$}. Then, we express the \er{intersection distance}
between these two paths as the Taylor expansion. Let's consider the following (see Figure~\ref{EqDifEpip}):
\begin{itemize}
   \item we make the quasi-vertical assumption\footnote{when this assumption can be made,
          \er{note we could use curvilinear abscissa}};
...
degenerate case for stereovision} and we must then \er{make of $\delta'_2 -\delta_2$ and $\delta'_1 -\delta_1$ term of degre $2$:}
...
are  lines $\LineK(v')$ in $E_k$, one can observe that $\phi'_1 \phi_1^{-1}$  and $\phi'_2 \phi_2^{-1}$ are diffeomorphisms transforming lines into lines, and they globally \er{operate the same transformation on lines.}
\textit{Vice versa}, let  $\phi_1,\phi_2$ be an epipolar resampling and let $\Lambda_1,\Lambda_2$ 
...
In this paper, the examples processed by our method are pushbroom satellite models known by their RPCs, the central perspective and \er{radar} models. However, the only restriction imposed on the generic nature of the model is that the projection function is "smooth", i.e.:
...
To begin with, the method estimates the centers \er{[inconsistent with small c in the implemention] $C_1,C_2$} of a set of points $p_1$ and $p_2$. This is done by calculating the average of all points' coordinates.  Then, the computations continue in
...
Next, for each image we estimate the average direction $\vec{D}_k$
of its epipolar lines, and using Equation~\eqref{EqRot}, a rotation $R_k$ is applied to the\er{data points ER does not like this data points}. The epipolar lines are now globally horizontal and the subsequent epipolar deformation is computed on the rotated data points. \er{ER is this phrase necessary? -The use of the coordinate system where epipolar lines are globaly horizontal, is
...
\end{equation}
%
\er{ER- remove that but add extra comment in the figure?
\begin{itemize}
...
$\mathcal{C}^{\infty}$ and, \er{according to the Stone-Weierstrass theorem \cite{Weierstrass1885,Stone1937} (which says that the space of polynomials is dense in the space of continuous functions),  with a sufficiently high
degree we will be able to accurately approximate any function}. A~possible
limitation of selecting high degree polynomial is over-fitting, which may lead to
unwanted high frequency behavior. In our case, this problem should never arise as the measurements  are synthesized from the projection functions $\pi_1,\pi_2$, which provides sufficient redundancy (for instance, hundreds of times more measurements than constraints).
...
When applying the constraint in Equation~\eqref{Ambig:Line}, we notice that  $i=0$, thence we can suppress all terms  $i^a$ for $a\neq 0$. The constraint equation then reads:
...
must be equal term by term. \er{The constraint then comes to force a number the
...
\subsection{Epipolar resampling without the geometric model -- \er{not too happy with this section}}
\label{EpipTieP}
...
if we have point correspondences between the image pairs but we don't know the geometric model?"} There is a straightforward answer to whether it is possible or not. As elaborated in the following pros and cons arguments, the answer depends on the point correspondences, and is a trade-off between the nature of the 3D scene and the complexity of the model. 
...
\er{As an archetype case}, for any point where the scene is not differentiable,
we have $ \DerPart{P}{u}= \infty$ and Equation~\eqref{DerParW2P} which lead to $ \DerPart{W_2} {u}=0$.
\er{Taking into account the degree freedom on $W_2$, $W_2$ is a polynomial function ..... missing?}
