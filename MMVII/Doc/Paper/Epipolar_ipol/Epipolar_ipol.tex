...
<<<<<<< HEAD
\subsection{Existence of epipolar geometry}\label{ExistEpip}
=======
\subsection{Existence of epipolar geometry}
\label{ExistEpip}
>>>>>>> c5f337490c1d0f667a982192fd79773efee0d10b
...
%
However, in general the existence of a stable foliation for the two bundles sets, as expressed in Equation~\eqref{ClothureBundle},
cannot  be satisfied. It is  illustrated 
by Figure~\ref{FigClothPath} and explained in the following.  Let $\pi_1,\pi_2$ be any two  projections and suppose
there exists  a foliation satisfying the Equation~\eqref{ClothureBundle}. Then, 

\begin{itemize}
   \item  let $P$ be any point in ground space, and $\Sv$ be the surface such that $P \in \Sv$;
...
\end{itemize}
In the general case, the above is a contradiction because there is no reason that for any two sets
...
\subsection{A local characterization of epipolar existence}
...
can be omitted for user interested mainly in the practical application.
...
as well as $\BundT$ and $\BundO$. Then, we express the intersection distance
between these two path as the Taylor expansion. Let's consider the following (see Figure~\ref{EqDifEpip}):
\begin{itemize}
...
Now, whatever may be the pair of 'small' values $(\delta_1,\delta_2)$,  we will compute a  pair of values
$(\delta'_1,\delta'_2)$ that minimize the distance $|Q_1,Q_2|$ and express the canceling of the
second degree Taylor expansion of this distance (the first degree can always being canceled out as we will see).  Noting $\delta$ the max of all $\delta$, the second degree Taylor expansion gives :
...
the deformation is made only on  $y$), as given in Equations~\eqref{Ambig:PhiO}
and~\ref{Ambig:PhiT};
\item Constraint on the global deformation of lines\footnote{i.e. where each line
is transformed globally to another line}. For instance, by fixing the transformation of one image, as given Equation~\eqref{FigAmbigEpip}.
\end{enumerate}
...
\end{equation}
%
This parametrization implements the constraints of Equations~\eqref{Ambig:PhiO} and~\eqref{Ambig:PhiT}. In Section~\ref{ChoicePolyn} we will see how the constraint of Equation~\eqref{Ambig:Line} is accounted for\footnote{See Equations~\eqref{CstrV1:0},~\eqref{CstrV1:1}.}. 
...
These models are considered as black-boxes that satisfy Equation~\eqref{Eq:Proj}, and for which no specific assumption is made on the physical model of the camera. In our own \CPP implementation,
the cameras are considered as pure virtual classes offering the interface to Equation~\eqref{Eq:Proj}.
In this paper, the examples processed by our method are RPC satellite models and the pinhole model, however, the
only restriction put is on the "smoothness" of the projection function, i.e.:
...
\end{figure}
%
\noindent Figure~\ref{BadGoodEpip} illustrates the latter constraint. The left image presents a set
...
To begin with, the method estimates the centers $C_1,C_2$ of a set of points $p_1$ and $p_2$. This is done by a calculating the average of all points coordinates.  Then, the computations pursue in
a coordinate system with the origin at that center. Thanks to this "normalisation", the constraint in Equation~\ref{Ambig:Line} will be applied at the center of the data points.

Next, for each image we estimate the average direction $\vec{D}_k$
of its epipolar lines, and using Equation~\eqref{EqRot} a rotation $R_k$ is applied to the data points. The epipolar lines become now globally horizontal and the subsequent epipolar deformation is computed on the rotated data points.
...
The use of the coordinate system where epipolar lines are globaly horizontal, is
...
\includegraphics[width=10cm]{FIGS/EpipReqOrient.jpg}
 \\ \hline \hline
\end{tabular}
\caption{Left : quasi vertical epipolar curve, correction with equation~\eqref{EpipVParam} is impossible. 
         Middle,Right  : oblique curve, epipolar rectification  with equation~\eqref{EpipVParam} is possible
...
Intuitively, it may be not obvious that the system of equation~\eqref{EqV1V2} is well posed.
...
And equation~\eqref{MultiTieP} brings in fact the constraint : 

\begin{equation}
V_2(p^1_2) = V_2(p^2_2)  =  V_2(p^3_2) \dots \label{RedrCurv}
\end{equation}

If we look at left image of figure~\ref{NonFuncCorresp}, we see that equation~\eqref{RedrCurv}
...
corresponding to coefficients of the polynoms as written in equation~\eqref{EqPol}:
...
In equation~\eqref{CstrV1:0}, $j$ and the  sum are both polynom, so if their function are equal on a segment, they
...
horizontal : we apply equation~\eqref{EqRot}  to all elements of the list.
...
\begin{itemize}
    ...
    \item the unknown are the coefficient of the polynomes $V_1$ and $V_2$, there is
          ...
          taking into account the  constraint of equation~\eqref{CstrV1:1}
     \item for each pair of corrected point $q_1,q_2$ we add to the least square system
          the equation~\eqref{EqPol};
\end{itemize}
...
there is \er{easy yes/no answer as there pro and cons arguments to solve this question}
...
\noindent {\underline {\bf Pro \& Cons:}} If  we analyse again the text of~\ref{WhyWork},
