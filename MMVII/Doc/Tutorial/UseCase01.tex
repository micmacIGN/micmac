...
This section presents how to add couple of clinometers to an image orientation. You are supposed to have :
\begin{itemize}
     \item a calibrated camera
     \item a set of images
     \item a set of measures acquired by a couple of clinometers in the same time than images
     \item a set of GCP points
\end{itemize}

The relative orientation between camera to the two clinometers are supposed to be constant through all acquisitions.
The aim of this calibration is to compute the relative orientations (Boresight matrixes) between camera and the two clinometers.
...
$B_1$ is the first Boresight matrix : the rotation from camera system to first clinometer system.

$B_2$ is the second Boresight matrix : the rotation from camera system to second clinometer system.

$B_1$ and $B_2$ are the unknowns of the problem.\\

The first clinometer measure is $\theta_1$ and its rotation matrix is $R_{\theta 1}$. 

The second clinometer measure is $\theta_2$ and its rotation matrix is $R_{\theta 2}$. \\

If first clinometer system is $(\Vec{i}, \Vec{j}, \Vec{k})$, $\theta_1$ is the measure of angle between $\Vec{i}$ and the vertical in the plan $(\Vec{i}, \Vec{j})$.\\
...
The vertical in the first clinometer system is $ B_1 R_C \Vec{v_{loc}} $\\

Each clinometer gives the angle between the vertical and one plan of its system.
Therefore, if you apply the two rotations given by the two clinometers, you should have the vertical $ \Vec{v} = \begin{pmatrix}
     1 \\
     0 \\
     0
\end{pmatrix}$\\
...
Rotation given by the first clinometer is applied on the vertical in the first clinometer system : $ R_{\theta 1} B_1 R_C \Vec{v_{loc}} $\\

Then, to apply the rotation given by the second clinometer, you need to go into the second clinometer system : $ R_{12} R_{\theta 1} B_1 R_C \Vec{v_{loc}} $ with $R_{12} = B_2 B_1^\intercal$\\

Now you can can apply the rotation given by the second clinometer $R_{\theta 2} R_{12} R_{\theta 1} B_1 R_C \Vec{v_{loc}} $\\

Finally :  
\begin{equation}
     R_{\theta 2} R_{12} R_{\theta 1} B_1 R_C \Vec{v_{loc}}  = \Vec{v}
\end{equation}

For the second clinometer :
\begin{equation}
     R_{\theta 1} R_{12}^\intercal R_{\theta 2} B_2 R_C \Vec{v_{loc}}  = \Vec{v}
\end{equation}
...
The first step is to find approximation of the two Boresight matrixes with {\tt ClinoInit}
...
Then, you can compute the two Boresight matrixes with {\tt OriBundleAdj}. You need to complete the parameter NameClino to compute the clinometers calibration
