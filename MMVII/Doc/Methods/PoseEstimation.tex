...
exactly the equation~\ref{EQ:PROJ}  from a set of $3$ correspondances. Other
chapter will discuss of how we use more (possibly many) correspondances to vbe
robust to outliers (for
example with Ransac) or to be more accurate in presence of "gaussian" noise (for 
example with least-squares).
...
Since rotation and translation are isometric, we have conservation of
distances between world and local coordinates. We can then write:
...
In equation~\ref{SpResecEQ:ConsDist}, $D_{kk'}$ and  $\Vec{u}_k$ are know, so we have $3$ equations 
$3$ unkowns.
...
implemanting this resolution. We call $\Vec{A} , \Vec{B}, \Vec{C}$  the bundles, $D_{AB}$ the distances etc.
...
Now we see that in equation~\ref{SpResecEQ:Lambda}, only the ratios $\frac{\lambda_{b}}{\lambda_{a}}$ and $\frac{\lambda_{c}}{\lambda_{a}}$
matter, so one can solve it as a system of $2$ equations with $2$ unknowns. Once we compute these ratios, the triangle
$(\Vec{A} , \frac{\lambda_{b}}{\lambda_{a}} \Vec{B},  \frac{\lambda_{c}}{\lambda_{a}} \Vec{C})$ will be proportional to the
