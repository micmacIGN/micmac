...
model that is continuously derivable as bicubic interpolation. However for efficiency reason,
we will generally prefer a non rigourous model as bilinear,  because the redundancy make
neglectable the difference with bicubic (later the bicubic option is planed to be added for finer
tests).

A typicall example is :

\begin{itemize}
      \item we have an image $I$, considered as a function $\RR^2 \rightarrow \RR$;
      \item we have a mapping of  $\RR^2$ ,  $\phi : \RR^2 \rightarrow \RR$.
      \item we want to use the composed function $I \circ \phi$;
\end{itemize}

Let $q$ , be a point and $q=(x_q,y_q)=\phi(p)$ its transformation by $\phi$. Let $x_0$ and $y_0$
be the lower bound of $q$ ,and $x_1=x_0+1, y_1 = y_0+1$:

\begin{equation}
	x_0 \leq x_q < x_1   \;\;  y_0 \leq y_q < y_1
\end{equation}
