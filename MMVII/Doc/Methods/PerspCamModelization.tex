...
of the parametric model, illustrated by figure ???  on a problem of curve interpolation:


\begin{itemize}
   \item if we don't have enough paramater we are not accurate  even on the data to fit,
	   here if we fit with a parabol we are not close to the learning data;

   \item if we  have too many parameter, we can have a very good match on the learning data, but have 
...
one universal rule : if can be good in interpolation, we are always bad in extrapolation. A 
consequence when will estimate the model of a camera : if we have no measurement in the corners,
we are almost sure that the model 

In the next section we are going to introduce step by step, different model with a continuous
refinement of the physical modelization.  At the end, the only hypothesis that we maintain from
equation~\ref{EqIntNoDist} is that all the bundle issued from image plane go through the same 
point $C$~\footnote{maybe at the end of {\tt MMVII} project we will test option to relax this hypothesis}.
