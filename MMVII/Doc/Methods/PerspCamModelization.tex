...
We will write $A\ll B$ for A is negligile with respect to B. We write $\nabla_F$
...
We have "proved" that $D^1 \circ D^2  \approx Id + \delta_1 + \delta_2$ .  Now it's trivial by recurence to
show that :
...
Using the odd polynom  of distorsion , we can write :
\begin{equation}
...
As the two distorsions have close centers, one is close to a radial distorsion
on the center of the other (even if their distorsion funcion are very different).
We take arbitray one of both as the "reference, here  $D_1$,
and we devlopp $D_2$ as a distorsion around the $C_1$ with polynom of $D^2_r$  plus an corrective term :
...
Note that have used $\vec{u_1}$ as point where the devlopment is made, we could have used $\vec{u_2}$,
...
we limit the radial distosion to the first term $k_1 \rho_1^2$.
So we have
...
Using standard convention, we name $\alpha = k_1  \delta_x $ abd $\beta = k_1  \delta_y$;
...
So we have shown that, to the first order, any combination of radial distorsion with slightly different centers,
