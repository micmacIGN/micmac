...
We want now to show that $D^1 \circ D^2  \approx Id + \delta_1 + \delta_2$ up to second order term.
We will write $A\ll B$ for A is negligile with respect to B. We write $\nabla_F$
the jacobian of $F$ and the taylor expansion :
...
\subsection{$2$ distorsion case}

Consider now $D_1$ and $D_2$ two  radial distorsion  :

\begin{equation}
	D^1 \circ D^2 (p)  \\
	= p +\delta^1(p) +\delta^2(p) 
	= p +   Pol_1(C_1-p) + Pol_2(C_2-p)
	= p +   Pol_1(C_1-p) + Pol_2(C_1-p)  +  ( Pol_2(C_2-p) - Pol_2(C_1-p))
	= p +   (Pol_1+ Pol_2)(C_1-p) +    ( Pol_2(C_2-p) - Pol_2(C_1-p))
	\approx p +   (Pol_1+ Pol_2)(C_1-p) +     (C_2-C_1)  \nabla (C2-p)
\end{equation}
