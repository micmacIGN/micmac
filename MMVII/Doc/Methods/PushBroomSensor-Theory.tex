...
\section{Physical and mathematical modelisation}
...
\subsection{Non physical model and reverse engenering}


The equation~\ref{PushB:EqPiM1}, and the devlopment deduced in the following sections,
assume that we have an explicit representation of the physical parameters
$\mathbb{P}=\{C(i),R(i),\mathbb{C}_{al}\}$.

However , as there is many format that can be fairly complex, it's very
common that the localisation are provided as non physical model that
...
to the required physical format because, with a bit of reverse
