...
cCollecSpecArg2007 & cAppliBundlAdj::ArgOpt(cCollecSpecArg2007 & anArgOpt) 
{
    ...
    return 
      ...
      << AOpt2007 ( mGCP3D, "GCP3D", "GCP ground coords and sigma factor, SG=0 fix, SG<0 schurr elim, SG>0 and optional output dir [[Folder,SigG,FOut?],...]]", {{eTA2007::ISizeV,"[2,3]"}})
      << AOpt2007 ( mGCP2D, "GCP2D", "GCP image coords and weight [[Folder,SigI,SigAt?=-1,Thrs?=-1,Exp?=1]...]",  {{eTA2007::ISizeV,"[2,4]"}})
...
void  cAppliBundlAdj::AddOneSetGCP3D(const std::string & aFolderIn, const std::string & aFolderOut, tREAL8 aWFactor)
{
    ...
    cMes3DDirInfo * aMesDirInfo = mBA.getGCP().addMes3DDirInfo(aFolderIn, aFolderOut, aWFactor);
...
void  cAppliBundlAdj::AddOneSetGCP2D(const std::vector<std::string> & aVParStd)
{
    ...
    cMes2DDirInfo * aMesDirInfo = mBA.getGCP().addMes2DDirInfo(aFolderIn, cStdWeighterResidual(aGCPW,1));
