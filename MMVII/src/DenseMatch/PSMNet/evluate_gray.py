...
if args.model == 'stackhourglass':
    model = stackhourglass(args.maxdisp, channel=1)
elif args.model == 'basic':
    model = basic(args.maxdisp, channel=1)
else:
    print('no model')
...
def main():
    ...
    for i in range(0, num):
        imgL_o = Image.open(all_left_img[i]).convert('L')
        imgR_o = Image.open(all_right_img[i]).convert('L')
