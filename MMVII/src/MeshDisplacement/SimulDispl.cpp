...
namespace MMVII
{
	...
	private:
		...
		std::string mInterpName;			  // Name of interpolator
	...
	cCollecSpecArg2007 &cAppli_SimulDispl::ArgOpt(cCollecSpecArg2007 &anArgOpt)
	{
		...
		return anArgOpt
			   ...
			   << AOpt2007(mInterpName, "Interp", "Interpolator type, \"Bilinear\", \"Linear\",\"Cubic\", \"SinCApod\", \"MMVIIK\" ", {eTA2007::HDV})
	...
	cDiffInterpolator1D *cAppli_SimulDispl::InitUserInterpolator()
	{
		...
		if (mInterpName == "Cubic")
			anInterp = cDiffInterpolator1D::AllocFromNames({"Tabul", "1000", "Cubic", "-0.5"});
		else if (mInterpName == "SinCApod")
			anInterp = cDiffInterpolator1D::AllocFromNames({"Tabul", "10000", "SinCApod", "10.0", "10.0"});
		else if (mInterpName == "MMVIIK")
			anInterp = cDiffInterpolator1D::AllocFromNames({"Tabul", "1000", "MMVIIK", "2.0"});
		else if (mInterpName == "Linear")
		{
		}
		else if (mInterpName == "Bilinear")
		{
		}
		else
