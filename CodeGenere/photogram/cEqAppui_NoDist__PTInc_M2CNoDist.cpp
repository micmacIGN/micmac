...
#include "cEqAppui_NoDist__PTInc_M2CNoDist.h"


cEqAppui_NoDist__PTInc_M2CNoDist::cEqAppui_NoDist__PTInc_M2CNoDist():
    cElCompiledFonc(2)
{
...
void cEqAppui_NoDist__PTInc_M2CNoDist::ComputeVal()
{
...
void cEqAppui_NoDist__PTInc_M2CNoDist::ComputeValDeriv()
{
   double tmp0_ = mCompCoord[0];
   double tmp1_ = mCompCoord[1];
   double tmp2_ = cos(tmp1_);
  ...
  mCompDer[0][0] = (tmp105_)*mLocNDdx_x+(tmp117_)*mLocNDdy_x;
  mCompDer[0][1] = (tmp128_)*mLocNDdx_x+(tmp140_)*mLocNDdy_x;
  mCompDer[0][2] = (tmp143_)*mLocNDdx_x+(tmp159_)*mLocNDdy_x;
  mCompDer[0][3] = (tmp164_)*mLocNDdx_x+(tmp169_)*mLocNDdy_x;
  mCompDer[0][4] = (tmp174_)*mLocNDdx_x+(tmp179_)*mLocNDdy_x;
  mCompDer[0][5] = (tmp183_)*mLocNDdx_x+(tmp188_)*mLocNDdy_x;
  mCompDer[0][6] = (tmp192_)*mLocNDdx_x+(tmp196_)*mLocNDdy_x;
  mCompDer[0][7] = (tmp200_)*mLocNDdx_x+(tmp204_)*mLocNDdy_x;
  mCompDer[0][8] = (tmp208_)*mLocNDdx_x+(tmp212_)*mLocNDdy_x;
  mVal[1] = (mLocNDP0_y+mLocNDdx_y*(tmp96_)+mLocNDdy_y*(tmp97_))-mLocYIm;

  mCompDer[1][0] = (tmp105_)*mLocNDdx_y+(tmp117_)*mLocNDdy_y;
  mCompDer[1][1] = (tmp128_)*mLocNDdx_y+(tmp140_)*mLocNDdy_y;
  mCompDer[1][2] = (tmp143_)*mLocNDdx_y+(tmp159_)*mLocNDdy_y;
  mCompDer[1][3] = (tmp164_)*mLocNDdx_y+(tmp169_)*mLocNDdy_y;
  mCompDer[1][4] = (tmp174_)*mLocNDdx_y+(tmp179_)*mLocNDdy_y;
  mCompDer[1][5] = (tmp183_)*mLocNDdx_y+(tmp188_)*mLocNDdy_y;
  mCompDer[1][6] = (tmp192_)*mLocNDdx_y+(tmp196_)*mLocNDdy_y;
  mCompDer[1][7] = (tmp200_)*mLocNDdx_y+(tmp204_)*mLocNDdy_y;
  mCompDer[1][8] = (tmp208_)*mLocNDdx_y+(tmp212_)*mLocNDdy_y;
}


void cEqAppui_NoDist__PTInc_M2CNoDist::ComputeValDerivHessian()
{
  ELISE_ASSERT(false,"Foncteur cEqAppui_NoDist__PTInc_M2CNoDist Has no Der Sec");
}

void cEqAppui_NoDist__PTInc_M2CNoDist::SetNDP0_x(double aVal){ mLocNDP0_x = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetNDP0_y(double aVal){ mLocNDP0_y = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetNDdx_x(double aVal){ mLocNDdx_x = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetNDdx_y(double aVal){ mLocNDdx_y = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetNDdy_x(double aVal){ mLocNDdy_x = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetNDdy_y(double aVal){ mLocNDdy_y = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetXIm(double aVal){ mLocXIm = aVal;}
void cEqAppui_NoDist__PTInc_M2CNoDist::SetYIm(double aVal){ mLocYIm = aVal;}



double * cEqAppui_NoDist__PTInc_M2CNoDist::AdrVarLocFromString(const std::string & aName)
{
...
cElCompiledFonc::cAutoAddEntry cEqAppui_NoDist__PTInc_M2CNoDist::mTheAuto("cEqAppui_NoDist__PTInc_M2CNoDist",cEqAppui_NoDist__PTInc_M2CNoDist::Alloc);


cElCompiledFonc *  cEqAppui_NoDist__PTInc_M2CNoDist::Alloc()
{  return new cEqAppui_NoDist__PTInc_M2CNoDist();
}
