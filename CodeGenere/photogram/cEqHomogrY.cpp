...
#include "StdAfx.h"
#include "cEqHomogrY.h"


cEqHomogrY::cEqHomogrY():
    cElCompiledFonc(1)
{
...
void cEqHomogrY::ComputeVal()
{

  mVal[0] = (mCompCoord[3]*mLocXL1+mCompCoord[4]*mLocYL1+mCompCoord[5])/(mCompCoord[6]*mLocXL1+mCompCoord[7]*mLocYL1+1)-(mCompCoord[11]*mLocXL2+mCompCoord[12]*mLocYL2+mCompCoord[13])/(mCompCoord[14]*mLocXL2+mCompCoord[15]*mLocYL2+1);

}


void cEqHomogrY::ComputeValDeriv()
{
  ...
  mCompDer[0][9] = 0;
  mCompDer[0][10] = 0;
  mCompDer[0][11] = -((mLocXL2*(tmp19_))/tmp20_);
  mCompDer[0][12] = -((mLocYL2*(tmp19_))/tmp20_);
  mCompDer[0][13] = -((tmp19_)/tmp20_);
  mCompDer[0][14] = -(-((tmp27_)*mLocXL2)/tmp20_);
  mCompDer[0][15] = -(-((tmp27_)*mLocYL2)/tmp20_);
}


void cEqHomogrY::ComputeValDerivHessian()
{
  ELISE_ASSERT(false,"Foncteur cEqHomogrY Has no Der Sec");
}

void cEqHomogrY::SetXL1(double aVal){ mLocXL1 = aVal;}
void cEqHomogrY::SetXL2(double aVal){ mLocXL2 = aVal;}
void cEqHomogrY::SetYL1(double aVal){ mLocYL1 = aVal;}
void cEqHomogrY::SetYL2(double aVal){ mLocYL2 = aVal;}



double * cEqHomogrY::AdrVarLocFromString(const std::string & aName)
{
...
cElCompiledFonc::cAutoAddEntry cEqHomogrY::mTheAuto("cEqHomogrY",cEqHomogrY::Alloc);


cElCompiledFonc *  cEqHomogrY::Alloc()
{  return new cEqHomogrY();
}
